...
- Date: 2026-09-02
- Decision: 增加一台电脑上的双人对战，第一版固定P1键盘、P2第一只手柄并提供第二套键盘备用；本地双人固定两名真人、无AI，使用同时框住两人的共享竞技场镜头。在线2–4人仍是正式产品的重要方向。
- Rationale: 核心战斗已经基本可玩，本地双人能以最低组织成本验证真实玩家对战、武器与回场乐趣，也便于线下演示。手柄支持比两人挤在同一键盘上更符合派对游戏场景。
- Consequences: 本决策明确取代D-002中“不提供同机多人”的部分，其他在线与原创性边界不变。Prototype 0.3.0先复用旧Input Manager验证玩法；手柄改键、多个手柄动态分配、震动和菜单手柄导航留待真人测试后决定。

## D-021 — 版本号按里程碑推进

- Date: 2026-09-02
- Decision: 版本号只在形成新的、连贯且可独立验证的开发里程碑时推进。同一里程碑中的bug修复、数值微调、输入修正、界面补全和连续试玩反馈都合并进当前版本日志，不为每次小改动增加补丁号。
- Rationale: 原型阶段反馈频繁，连续使用0.3.1、0.3.2会让版本号失去“可识别里程碑”的意义，也增加Codex、Claude、本地文件和GitHub之间的同步成本。
- Consequences: 本地双人/三人、LT/RT、物理后坐力和显示设置统一属于Prototype 0.3.0。只有当前批次稳定并开始一个新的成组目标时，才选择下一个版本号。
